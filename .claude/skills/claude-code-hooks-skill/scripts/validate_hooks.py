...
def validate_hook(hook: dict[str, Any], path: str, event: str) -> list[ValidationError]:
    ...
    if not hook_type:
        ...
        errors.append(ValidationError(path, f"Invalid type '{hook_type}', must be 'command' or 'prompt'"))
    
    # Type-specific validation
    if hook_type == 'command':
        if 'command' not in hook:
            errors.append(ValidationError(path, "Missing 'command' field for type='command'"))
        ...
        if 'prompt' not in hook:
            errors.append(ValidationError(path, "Missing 'prompt' field for type='prompt'"))
    ...
    if 'timeout' in hook:
        ...
        if not isinstance(timeout, (int, float)):
            errors.append(ValidationError(path, f"Timeout must be a number, got {type(timeout).__name__}"))
        elif timeout <= 0:
    ...
    for field in hook:
        if field not in known_fields:
            errors.append(ValidationError(path, f"Unknown field '{field}'", severity='warning'))
...
def validate_matcher_group(
    ...
    if event == 'Notification' and matcher not in NOTIFICATION_MATCHERS and matcher != '*' and matcher != '':
        errors.append(ValidationError(
            path,
...
def validate_hooks(data: dict[str, Any]) -> list[ValidationError]:
    ...
    for event_name, event_matchers in hooks_obj.items():
        ...
        if len(event_matchers) == 0:
            errors.append(ValidationError(event_path, "Empty event array", severity='warning'))
            continue
        ...
        for i, matcher_group in enumerate(event_matchers):
            ...
            if not isinstance(matcher_group, dict):
                errors.append(ValidationError(group_path, "Matcher group must be an object"))
                continue
